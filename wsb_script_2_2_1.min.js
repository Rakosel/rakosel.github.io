var ra,rs,lines_in,i,url,maOBJ,seOBJ,bmeOBJ,sds,mds,sets,stopAll=!1,subwdeb=!1,str_out="",str_out1="",uart_json={},temp_json={},input_lm75={},scrPos=0,ovShBtn0=!1,ast=0,ua_mode=0,tmranim=3E3;sds=$(".sideset");mds=$(".macnt");sets=$(".setcnt");cnftmp=$(".scntf");function btn_otp_wr(){var b="/otp_mode?input="+encodeURIComponent("Wr345")+"&";fetch(b,"GET",txjstmp,20)}
function btn_sensor_rd(){var b,f=$("#btnsens_val").val(),c=0;if(1!=ast)for(c=0;c<f;c++)b="/otp_mode?input="+encodeURIComponent("GiveMeas")+"&",fetch(b,"GET",txjstmp,40)}function btn_otp_rd(){var b="/otp_mode?input="+encodeURIComponent("Rd345")+"&";fetch(b,"GET",txjstmp,20)}function refr_rtc(){0==subwdeb&&$("#autmp").prop("checked")&&(fetch("/get_rtc.json?n="+Math.random(),"GET",txjstmp,10),sub_grad())}
...
window.onload=function(){$(".bt0st").attr("value","off");$(".navia").addClass("list-group-item list-group-item-action bg-light border");$("#esp_tx").val("wsbuser.prints(node.heap());");$("#esp_urx").val("");rs=setInterval(refr_rtc,5000);i=0;smgh()};
