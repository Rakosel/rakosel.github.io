// upd79a7 STABLE trim with upravl timer	https://rakosel.github.io/wsb_script_2_2_1.js
// 682 stroka trim ???
// except (
...
//    console.log(d);
    ...
    if (temp_json["temp"]) {
        ...
        for (i = 0; i <= temp_json.temp.length; i++) {
            //try {
				if(i<=T_arr.length && temp_arr[T_arr[i]] != "#ERR" && temp_arr[T_arr[i]] != "" && temp[T_arr[i]] != isNaN())
				{
					j_T+=parseFloat(temp_arr[i]);T_cnt++;
				}
				if(i<=H_arr.length && temp_arr[H_arr[i]] != "#ERR" && temp_arr[H_arr[i]] != "" && temp[H_arr[i]] != isNaN())
				{
					j_H+=parseFloat(temp_arr[i]);H_cnt++;
				}
				if(i<=P_arr.length && temp_arr[P_arr[i]] != "#ERR" && temp_arr[P_arr[i]] != "" && temp[P_arr[i]] != isNaN())
				{
