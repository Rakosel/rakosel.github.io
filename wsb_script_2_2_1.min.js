// upd79a9 STABLE trim with upravl timer	https://rakosel.github.io/wsb_script_2_2_1.js
// 682 stroka trim ???
// except (
...
//    console.log(d);
    ...
    if (temp_json["temp"]) {
        ...
        for (i = 0; i <= temp_json.temp.length; i++) {
            //try {
				if(i<=T_arr.length && temp_arr[T_arr[i]] != "#ERR" && temp_arr[T_arr[i]] != "" && temp_arr[T_arr[i]] != isNaN())
				{
					j_T+=parseFloat(temp_arr[i]);T_cnt++;
				}
				if(i<=H_arr.length && temp_arr[H_arr[i]] != "#ERR" && temp_arr[H_arr[i]] != "" && temp_arr[H_arr[i]] != isNaN())
				{
					j_H+=parseFloat(temp_arr[i]);H_cnt++;
				}
				if(i<=P_arr.length && temp_arr[P_arr[i]] != "#ERR" && temp_arr[P_arr[i]] != "" && temp_arr[P_arr[i]] != isNaN())
				{
					j_P+=parseFloat(temp_arr[i]);P_cnt++;
				}	
        }
		//console.log(+"a5 "+j_T+" "+j_H+" "+j_P+" "+T_cnt+" "+H_cnt+" "+P_cnt);
		j_T=j_T/T_cnt; j_H=j_H/H_cnt; j_P=j_P/P_cnt;
		
		 $("#temperature").val(j_T.substring(0, 5));
		 $("#humudity").val(j_H.substring(0, 5));
		 $("#pressure").val(j_P.substring(0, 5));
