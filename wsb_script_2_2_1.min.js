// upd77a1 STABLE trim with upravl timer	https://rakosel.github.io/wsb_script_2_2_1.js
// 682 stroka trim ???
// except (
...
//    console.log(d);
    ...
    if (temp_json["temp"]) {
        for (i = 3; i <= maOBJ.length && (i - 3) <= temp_json.temp.length; i++) {
            ...
            //str_out = temp_json.temp[i - 3].toString().trim();
            $("#" + maOBJ[i].name).val(temp_json.temp[i - 3].toString().trim());
        } 
    }
    $("#tm_adc").removeClass("is-invalid").html();
    $("#tm_adc").removeClass("is-valid").html();
    if (temp_json["temt_adc"]) {
        $("#tm_adc").val(temp_json.temt_adc.toString().trim());
