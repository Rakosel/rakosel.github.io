// upd78a7 STABLE trim with upravl timer	https://rakosel.github.io/wsb_script_2_2_1.js
// 682 stroka trim ???
// except (
...
//    console.log(d);
    ...
    if (temp_json["temp"]) {
        ...
        for (i = 0; i <= temp_json.temp.length; i++) {
            //try {
				if(i<=T_arr.length || temp_json.temp[T_arr[i]] != "#ERR" || temp_json.temp[T_arr[i]] != "" || temp_json.temp[T_arr[i]] != isNaN())
				{
					j_T+=temp_json.temp[i];T_cnt++;
				}
				if(i<=H_arr.length || temp_json.temp[H_arr[i]] != "#ERR" || temp_json.temp[H_arr[i]] != "" || temp_json.temp[H_arr[i]] != isNaN())
				{
					j_H+=temp_json.temp[i];H_cnt++;
				}
				if(i<=P_arr.length || temp_json.temp[P_arr[i]] != "#ERR" || temp_json.temp[P_arr[i]] != "" || temp_json.temp[P_arr[i]] != isNaN())
				{
					j_P+=temp_json.temp[i];P_cnt++;
				}	
        }
		console.log(j_T+" "+j_H+" "+j_P+" "+T_cnt+" "+H_cnt+" "+P_cnt);
		j_T=j_T/T_cnt; j_H=j_H/H_cnt; j_P=j_P/P_cnt;
		
		 $("#temperature").val(j_T/T_cnt);
		 $("#humudity").val(j_H/H_cnt);
		 $("#pressure").val(j_P/P_cnt);
